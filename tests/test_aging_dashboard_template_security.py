...
def test_aging_dashboard_uses_safe_dom_nodes_for_dynamic_contact_text():
    ...
    assert "function createChartSegment(className, widthPercent, title)" in source
    assert "segment.style.width = `${widthPercent}%`;" in source
    assert "function safePercent(value)" in source
